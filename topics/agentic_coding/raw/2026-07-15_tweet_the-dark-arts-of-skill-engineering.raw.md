...
URL: https://x.com/i/status/2077114326985687525
Canonical URL: https://x.com/pbakaus/status/2077114326985687525
Author: Paul Bakaus (@pbakaus)
Article title: The Dark Arts of Skill Engineering
Preview: It's trivial to create skills. It's really hard to create effective skills. I learned this the hard way, and on my journey picked up some truly arcane knowledge that helped me level up Impeccable from
Cover image: https://pbs.twimg.com/media/HNNbvr8agAA4zj_.jpg
...
1. Force divergence · escape the cluster, don't chase it with bans
1. Route like a frontier model · one command loads one expert
1. Give them memory · runs that compound instead of restarting
1. Scripts that talk back · stdout is the instruction
1. Hooks that fight back · guardrails that fire uninvited
1. Live-wire the browser · the browser becomes a second input
1. Compile to every harness · one source, every runtime, every model
1. Design for the weakest model · gates that can't be skipped
...
1. detector silent → rubber-stamps a generic page (meaningless clean bill)
...
1. Generate ~50, a blind sub-agent keeps the 5 most distinct. (Lives in Radiant's codebase, not Impeccable yet.)
1. Seed from outside the model: `palette.mjs --from 8f2a` starts in a region the model never picks on its own.
...
1. /polish reads the latest snapshot as its fix backlog
...
1. The skill body says "follow whatever the script prints."
1. The script reads the environment and writes the next instruction back: context.mjs prints NO_PRODUCT_MD, and the agent stops and runs init.
...
1. If the file is ignored or unrelated to frontend dev, the script no-ops.
1. Otherwise, Impeccable's deterministic slop engine scans the file and injects findings as a system reminder.
1. The agent fixes all remaining issues.
...
1. The agent runs ONE blocking command (live-poll.mjs) and waits.
1. You select and hit generate in the running app → that becomes the event the poll returns.
1. The agent, in the same chat thread, edits the file and replies; the page updates.

---
type: tweet
source_url: "https://x.com/i/status/2023452909883609111"
canonical_url: "https://x.com/ryancarson/status/2023452909883609111"
title: Code Factory — repo control-plane for coding agents
author: Ryan Carson
handle: ryancarson
created_at: 2026-02-18
topics: [agentic_coding]
content_hash: c50bb2bd6f1726f28963ced54baa033cc6342b5d9d53f74c0b7e431310af18f7
extracted_at: "2026-02-18T18:48:58"
extractor: fxtwitter
date: 2026-02-18
timestamp: 2026-02-18
resource: "https://x.com/ryancarson/status/2023452909883609111"
---

# Raw content

Source: https://x.com/ryancarson/status/2023452909883609111


Source: https://x.com/ryancarson/status/2023452909883609111

Author: Ryan Carson (@ryancarson)
Tweet created_at: 2026-02-16
Tweet text (as posted): https://t.co/c0jGiIrUke

This post is essentially a link-post to the attached X Article.

Article title: Code Factory: How to setup your repo so your agent can auto write and review 100% of your code
Article preview:
The goal
You want one loop:
1) The coding agent writes code
2) The repo enforces risk-aware checks before merge
3) A code review agent validates the PR
4) Evidence (tests + browser + review) is machine-verifiable
5) Findings turn into repeatable harness cases

Mentions in article: @greptile, @coderabbitai
Inspiration credit: @_lopopolo

Primary links:
- Tweet: https://x.com/ryancarson/status/2023452909883609111
- X Article canonical (tweet embed): https://x.com/i/status/2023452909883609111
- Cover image: https://pbs.twimg.com/media/HBS_KCrXwAA-N_q.jpg

Note: Full article content is long; extracted via FxTwitter JSON blocks (truncated in this capture). Re-open FxTwitter payload if you want the entire article dumped verbatim.
